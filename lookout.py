...
def _generate_progress_bar(position, max_value):
    ...
    empty_char = "⬜"
    return (
        (filled_char * int(filled_squares))
...
class KubeLookout:
    template = [
        {"type": "section", "text": {"type": "mrkdwn", "text": ""}},
        {
            ...
            "accessory": {"type": "image", "image_url": "", "alt_text": "status image"},
        },
    ]
    ...
    def _send_slack_block(self, blocks, channel, message_id=None):
        ...
        response = self.slack_client.chat_update(
            channel=channel, ts=message_id, blocks=blocks
        )
        return response.data["ts"], response.data["channel"]
    ...
    def _handle_deployment_change(self, deployment):
        ...
        if (
            deployment_key not in self.rollouts
            and deployment.status.updated_replicas is None
        ):
            ...
            deployment_key in self.degraded
            and ready_replicas >= deployment.spec.replicas
        ):
    ...
    def _generate_deployment_rollout_block(self, deployment, rollout_complete=False):
        ...
        header = (
            f"*{self.cluster_name} deployment "
        ...
        for container in deployment.spec.template.spec.containers:
            message += (
                f"Container {container.name} has image " f"_ {container.image} _\n"
            )
        message += "\n"
        message += (
            f"{deployment.status.updated_replicas} replicas "
            f"updated out of "
...
if __name__ == "__main__":
    ...
    env_progress_image = os.environ.get(
        "PROGRESS_IMAGE", "https://i.gifer.com/80ZN.gif"
    )
    env_ok_image = os.environ.get(
        "OK_IMAGE",
        "https://upload.wikimedia.org/wikipedia/"
